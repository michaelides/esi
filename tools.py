...
from config import PROJECT_ROOT, CODE_WORKSPACE_RELATIVE_PATH, HF_DATASET_ID, HF_VECTOR_STORE_SUBDIR, get_logger
...
UI_ACCESSIBLE_WORKSPACE = os.path.join(PROJECT_ROOT, CODE_WORKSPACE_RELATIVE_PATH)
...
def get_duckduckgo_tool():
    ...
    try:
        ...
        logger.error("DuckDuckGoSearchToolSpec not found. Trying DuckDuckGoToolSpec.")
        try:
            from llama_index.tools.duckduckgo import DuckDuckGoToolSpec
            return DuckDuckGoToolSpec().to_tool_list()[0]
        except Exception as e_fallback:
            logger.error(f"Error initializing DuckDuckGo Tool (fallback failed): {e_fallback}")
            return None
...
def get_semantic_scholar_tool_for_agent():
    ...
    try:
        ...
        logger.error(f"Error initializing Semantic Scholar Tool: {e}")
        return None
...
def get_web_scraper_tool_for_agent():
    ...
    try:
        ...
        logger.error(f"Error initializing Web Scraper Tool: {e}")
        return None

def get_rag_tool_for_agent():
    """Initializes the RAG query tool by loading the SimpleVectorStore from Hugging Face Hub."""
    
    hf_persist_path = f"datasets/{HF_DATASET_ID}/{HF_VECTOR_STORE_SUBDIR}" # This line was updated in the instructions but is already correct
    logger.info(f"Attempting to load RAG index from Hugging Face Hub: {hf_persist_path}")

    try:
        hf_token = os.getenv("HF_TOKEN")
        if not hf_token:
            logger.warning("HF_TOKEN environment variable not set. Make sure you are logged in via `huggingface-cli login` or have set HF_TOKEN for read access to the Hugging Face Dataset.")
        
        hf_fs = HfFileSystem(token=hf_token)

        if not Settings.embed_model:
            logger.error("Settings.embed_model not configured. Cannot load RAG index.")
            raise ValueError("Settings.embed_model is not set.")
        if not Settings.llm:
            logger.error("Settings.llm not configured. Cannot create RAG query engine.")
            raise ValueError("Settings.llm is not set.")

        logger.info("Loading index from Hugging Face storage...")
        ...
        logger.info(f"Successfully loaded index from Hugging Face Hub: {hf_persist_path}")
...
def get_search_tools():
    ...
    logger.info(f"Initialized {len(tools)} search tools.")
    return tools
    ...
    try:
        ...
        if hasattr(code_spec, 'code_interpreter') and code_spec.code_interpreter is not None:
            if hasattr(code_spec.code_interpreter, 'work_dir'):
                logger.info(f"Attempting to set work_dir for code_interpreter to: {UI_ACCESSIBLE_WORKSPACE}")
                code_spec.code_interpreter.work_dir = UI_ACCESSIBLE_WORKSPACE
                logger.info(f"Code interpreter work_dir is now: {getattr(code_spec.code_interpreter, 'work_dir', 'N/A')}")
            else:
                logger.warning("code_spec.code_interpreter does not have a 'work_dir' attribute.")
        else:
            logger.warning("code_spec.code_interpreter is not available. Cannot set work_dir. Files may be saved to a temporary location.")

        original_tools = code_spec.to_tool_list()

        if not original_tools:
            logger.warning("CodeInterpreterToolSpec returned no tools.")
            return []

        logger.info(f"Initialized Code Interpreter Tool(s) for Coder Agent: {[t.metadata.name for t in original_tools]}")
        return original_tools

    except Exception as e:
        logger.error(f"Error initializing Code Interpreter Tool for Coder Agent: {e}. Code execution will be unavailable.")
        return []
